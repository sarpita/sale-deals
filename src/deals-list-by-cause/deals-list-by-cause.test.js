...
import DealsListByCause from "./index";
...
describe('Deals List By Cause component', () => {

  test('show deals list group by causes', async () => {
    useSelector.mockImplementation(() => (dealsMock));
    const {getByText, container} = render(<BrowserRouter><DealsListByCause/></BrowserRouter>);
    console.log(container.innerHTML);
    expect(getByText('Deals List - grouped by cause')).toBeInTheDocument();
    expect(getByText('Science')).toBeInTheDocument();
    expect(getByText('Make your website\'s home page look good on small devices')).toBeInTheDocument();
    expect(getByText('1 hour Photoshoot session')).toBeInTheDocument();
    expect(getByText('Creative Brainstorming Session')).toBeInTheDocument();

    expect(getByText('Children\'s Rights')).toBeInTheDocument();
    expect(getByText('Basic Introduction to Javascript')).toBeInTheDocument();

    expect(getByText('Environment')).toBeInTheDocument();
    expect(getByText('3 Donut Bar Stools - 30" Seat Height and Swivel')).toBeInTheDocument();
    expect(getByText('One Private Yin Yoga Lesson')).toBeInTheDocument();
    expect(getByText('Thai Yoga Therapy - 7-Day Program')).toBeInTheDocument();
  })
